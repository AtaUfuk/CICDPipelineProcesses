...
### 6.1) Ek Deploy Örneği (Windows Server + IIS)

Bu ek örnekte deploy adımı Linux yerine Windows Server + IIS hedefi için gösterilir. Kimlik bilgileri yine variable group üzerinden alınır.

```yaml
- stage: Deploy_Stage_Windows_IIS
  displayName: Deploy to Stage (Windows IIS)
  dependsOn: Build_Test_Publish
  condition: succeeded()
  jobs:
    - deployment: DeployToWindowsIIS
      displayName: Backup, Deploy and Restart IIS
      environment: stage
      pool:
        vmImage: windows-latest
      strategy:
        runOnce:
          deploy:
            steps:
              ...
              - powershell: |
                  $secure = ConvertTo-SecureString "$(WIN_PASSWORD)" -AsPlainText -Force
                  $cred = New-Object System.Management.Automation.PSCredential("$(WIN_USERNAME)", $secure)
                  $session = New-PSSession -ComputerName "$(WIN_HOST)" -Credential $cred

                  Invoke-Command -Session $session -ScriptBlock {
                    param($IisSiteName, $IisAppPool, $DeployPath, $BackupRoot)
                    $now = Get-Date -Format "yyyyMMdd_HHmmss"
                    New-Item -ItemType Directory -Path $BackupRoot -Force | Out-Null
                    if (Test-Path $DeployPath) {
                      $zipPath = Join-Path $BackupRoot ("app_backup_" + $now + ".zip")
                  ...
                  } -ArgumentList "$(IIS_SITE_NAME)", "$(IIS_APPPOOL)", "$(DEPLOY_PATH)", "$(BACKUP_ROOT)"

                  Copy-Item -Path "$(Pipeline.Workspace)\$(artifactName)\*" -Destination "$(DEPLOY_PATH)" -ToSession $session -Recurse -Force

                  Invoke-Command -Session $session -ScriptBlock {
                    param($IisSiteName, $IisAppPool)
                    Import-Module WebAdministration
                    Start-WebAppPool -Name $IisAppPool
                    Start-Website -Name $IisSiteName
                  } -ArgumentList "$(IIS_SITE_NAME)", "$(IIS_APPPOOL)"

                  Remove-PSSession $session
                displayName: Backup, deploy and restart IIS site
```

Beklenen ek variable group değişkenleri (Windows/IIS):
- `WIN_HOST`
- `WIN_USERNAME`
- `WIN_PASSWORD` (secret)
- `IIS_SITE_NAME`
- `IIS_APPPOOL`
- `DEPLOY_PATH` (ör. `C:\inetpub\wwwroot\my-app`)
- `BACKUP_ROOT` (ör. `D:\deploy-backups\my-app`)
...
### 6.1) Additional Deploy Sample (Windows Server + IIS)

This sample shows the deployment step for Windows Server + IIS instead of Linux. Credentials are still read from a variable group.
...
Expected additional variable group values (Windows/IIS):
- `WIN_HOST`
- `WIN_USERNAME`
- `WIN_PASSWORD` (secret)
- `IIS_SITE_NAME`
- `IIS_APPPOOL`
- `DEPLOY_PATH` (for example `C:\inetpub\wwwroot\my-app`)
- `BACKUP_ROOT` (for example `D:\deploy-backups\my-app`)
